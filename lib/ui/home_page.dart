...
import 'package:flutter_modularization/ui/other_options_page.dart';
...
class _HomePageState extends State<HomePage> {
  ...
  int setValue(value) {
    int val=0;
    value == null? null : val = value+1;
    return value;
  }

  Widget showListMovie(AsyncSnapshot<Movie> snapshot) => ListView.builder(
        itemCount: snapshot.data == null ? 0 : addOne(snapshot.data?.results.length),
        itemBuilder: (BuildContext context, int index) {
          final movie = snapshot.data!.results[index];
          return index == setValue(snapshot.data?.results.length)? GestureDetector(
            ...
            onPressed: () {
              Navigator.push(
                context,
                MaterialPageRoute(builder: (context) => const OtherOptionsPage()));
            },
